...
import { buildTurnReview, findReviewWindowStart } from '../turnReview'
...
function makeGenesis(units: Unit[], board: GameState['board'], extraDeclineForP1: string[] = []): GameState {
  ...
  const active: GameState = { ...lobby, board, players: [p1, makePlayer('p2', p2Cards)], cards, turnOrder: ['p1', 'p2'], units, status: 'active' }
