...
describe('round flow', () => {
  it('runs select -> actions -> purchase -> next round when no decline is triggered', () => {
    ...
    result = applyAction(result.state, { type: 'RESOLVE_UNIT_ACTION', playerId: 'p1', actionIdByUnitId: {} }, testUnitContent)
    if (!result.ok) throw new Error('setup failed')
    result = applyAction(result.state, { type: 'RESOLVE_UNIT_ACTION', playerId: 'p2', actionIdByUnitId: {} }, testUnitContent)
    expect(result.ok).toBe(true)
    if (!result.ok) return
