...
describe('RoundView — stacked units on one hex (Ship + Port, The Ports Tale)', () => {
  ...
  function renderStacked() {
    ...
    // p2 has no units, so no cards ever reach their hand and they're
    // auto-eliminated at beginSelectCardsPhase — same pattern the
    // Convert-to-Merchant test above relies on, so p1's single CHOOSE_CARD
    // is enough to reach the actions phase.
    const active: GameState = { ...lobby, board, units: [ship, port], status: 'active' }
    const selecting = beginSelectCardsPhase(syncCardZonesWithBoard(active))
