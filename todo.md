...
## 56. The Ports Tale (variant) — first Tale implemented, plus the general "companion piece" engine capability it needed
...
## 58. Round-play UI for companion pieces (Port) — closes #57's scope note, plus a stacked-hex rendering bug it surfaced

Two related requests: make a Tale companion piece (Port) actually
clickable/usable in the live round UI (the gap #57 flagged), and fix what
...
companion of Ship) was invisible to it even once #56 made the engine
...
`applyResolveUnitAction` rule from #56). Used everywhere `RoundView.tsx`
...
routed to the right unit id). 434 tests total (was 427); `tsc -b`/
